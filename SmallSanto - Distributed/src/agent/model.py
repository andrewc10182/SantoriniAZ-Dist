...
class GameModel:
    ...
    def build(self):
        ...
        res_out = x
        # for policy output
        x = Conv2D(filters=4, kernel_size=1, data_format="channels_first", kernel_regularizer=l2(mc.l2_reg))(res_out)
        x = BatchNormalization(axis=1)(x)
        x = Activation("relu")(x)
        x = Flatten()(x)
        # no output for 'pass'
        ...
        # for value output
        x = Conv2D(filters=16, kernel_size=1, data_format="channels_first", kernel_regularizer=l2(mc.l2_reg))(res_out)
        x = BatchNormalization(axis=1)(x)
        x = Activation("relu")(x)
        x = Flatten()(x)
...
##        plot_model(self.model, to_file='model.png', show_shapes=True)
    ...
    def load(self, config_path, weight_path):
        if os.path.exists(config_path) and os.path.exists(weight_path):
            ...
            print("model files does not exist at ",config_path," and ",weight_path)
            return False
